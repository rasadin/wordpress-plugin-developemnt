...
module.exports = function( grunt ) {
    ...
    grunt.initConfig({
            ...
            my_target: {
                files: {
                    ...
                    '<%= dirs.public_dist %>/js/public.min.js': [
                        '<%= dirs.public_source %>/js/provision.js',
                        '<%= dirs.public_source %>/js/public.js',
                    ],

                    '<%= dirs.public_dist %>/js/jquery.validate.min.js': [
                        '<%= dirs.public_source %>/js/jquery.validate.js',
                    ],

                    '<%= dirs.public_dist %>/js/additional-methods.min.js': [
                        '<%= dirs.public_source %>/js/additional-methods.min.js',
                    ],
        ...
        makepot: {
            target: {
                options: {
                    ...
                    potFilename: 'webcommander-core.pot', // Name of the POT file.
                    type: 'wp-plugin', // Type of project (wp-plugin or wp-theme).
                    potHeaders: {
                        'report-msgid-bugs-to': 'https://webalive.com.au',
        ...
        compress: {
            main: {
                options: {
                    mode: 'zip',
                    archive: './build/webcommander-core'+ pkg.version + '.zip'
                },
                expand: true,
                cwd: 'build/',
                src: ['**/*'],
                dest: 'webcommander-core'
            }
        },
    });
    ...
    grunt.registerTask( 'default', [
       'clean', 'minifycss', 'minifyjs', 'watcher'
    ]);
